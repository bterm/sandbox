...
-(void) checkAndCreateDatabase:(NSString *)databasePath db:(NSString *)dbname {
	BOOL success;
	...
	success = [fileManager fileExistsAtPath:[databasePath stringByAppendingPathComponent:dbname]];
...
- (BOOL)open:(NSString *)name {
	...
	// DB stored in the documents directory, get that path from the file system
	NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
	
	// see if the db exists on the file system, if not copy it over from the application
	[self checkAndCreateDatabase:documentsDirectory db:name];
	
	// full path the the db
    NSString *writableDBPath = [documentsDirectory stringByAppendingPathComponent:name];
		
	// Attempt to open the db
	if (sqlite3_open([writableDBPath fileSystemRepresentation], &_db) != SQLITE_OK) {
		NSLog(@"SQLite Opening Error: %s", sqlite3_errmsg(_db));
		return NO;
	}

	filePath = [writableDBPath retain];
	return YES;
}
